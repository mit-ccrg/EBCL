"""Duett  from https://github.com/layer6ai-labs/DuETT/blob/master/duett.py"""
import numpy as np
...
from xformers.components.feedforward import Feedforward, FeedforwardConfig, register_feedforward

@dataclass
class CustomMlpConfig(FeedforwardConfig):
    hidden_dim: int
    bias: bool


@register_feedforward("CustomMLP", CustomMlpConfig)
class CustomMLP(Feedforward):
    def __init__(
        self,
        dim_model: int,
        dropout: float,
...
def make_transformer_layer(dim, dropout, n_heads, seq_len, d_feedforward):
    encoder_config = {
        "dim_model": dim,
        "num_layers": 1,
        ...
        "multi_head_config": {
            ...
            "attention": {
                "name": "scaled_dot_product",  # attention mechanism
                "dropout": dropout,
                "seq_len": seq_len,
            },
        },
        "feedforward_config": {
            ...
            "dropout": dropout,
            "activation": "relu",
            "hidden_dim": d_feedforward,
        },
    }
    transformer = xFormerEncoderBlock(xFormerEncoderConfig(**encoder_config))
    return transformer
    ...
    def forward(self, x):
        if x.ndim == 2:
            ...
            raise NotImplementedError(
                "BatchNormLastDim not implemented for ndim > 3 yet"
            )
    ...
    if n_hidden == 0:
        ...
        layers = (
            ...
            + [
                ...
                for l in ([BatchNormLastDim(d_hidden)] if hidden_batch_norm else [])
...
def initialize_pretrain_duett(cfg: DuettModelConfig):
    """Load Duett Pretraining Model

    Args:
        d_time_series_num (int): _description_
        d_target (int): _description_

    Returns:
        DuettModule: Duett Pretraining Model
    """
    assert cfg.pretrain, "Duett Pretraining Model must be loaded with pretrain=True"
    d_time_series_num = cfg.num_event
    d_target = 1  # binary classification
    return DuettModule(
        d_time_series_num, d_target,
        half_dtype=cfg.half_dtype,
        pretrain_dropout=cfg.dropout,
        lr=cfg.lr)


def load_fine_tune_duett(cfg: DuettModelConfig, ckpt_path: str):
    """Load Duett Model for Fine-Tuning.

    Args:
        ckpt_path (str): path to model checkpoint to load

    Returns:
        DuettModule: Loads a Pretrained Duett model that is ready for finetuning.
    """
    assert (
        not cfg.pretrain
    ), "Duett Fine-Tuning Model must be loaded with pretrain=False"

    d_time_series_num = cfg.num_event
    d_target = 1  # binary classification

    return DuettModule.load_from_checkpoint(
        ckpt_path,
        ...
        aug_noise=0.0,
        aug_mask=0.5,
        transformer_dropout=cfg.dropout,
        lr=cfg.lr,
        weight_decay=1.0e-5,
...
class DuettModule(pl.LightningModule):
    def __init__(
        self,
        d_time_series_num,
        d_target,
        ...
        **kwargs,
        ...
        self.register_buffer(
            "MASKED_EMBEDDING_KEY", torch.tensor(0)
        ...
        self.embedding_layers = nn.ModuleList(
            [
                simple_mlp(
                    2,
                    d_embedding,
                    n_hidden_mlp_embedding,
                    d_hidden_mlp_embedding,
                    hidden_batch_norm=True,
                )
        ...
        tt_dim = d_embedding * (d_time_series_num)  # TODO
        self.event_transformers = nn.ModuleList(
            [
                # make_transformer_layer(
                #     dim=et_dim,
                #     dropout=transformer_dropout,
                #     n_heads=n_transformer_head,
                #     seq_len=d_time_series_num,
        ...
        self.full_event_embedding = nn.Embedding(d_time_series_num, et_dim)  # TODO
        self.time_transformers = nn.ModuleList(
            [
                ...
                #     dropout=transformer_dropout,
                #     n_heads=n_transformer_head,
                #     seq_len=masked_transform_timesteps + 1,
                #     d_feedforward=d_feedforward)
                x_transformers.Encoder(
                    dim=tt_dim,
                    depth=1,
        ...
        d_representation = d_embedding * (
            d_time_series_num
        )  # time_series + static # TODO
        ...
        self.train_auroc = torchmetrics.AUROC(num_classes=num_classes, task="binary")
        self.train_acc = torchmetrics.Accuracy(num_classes=num_classes, task="binary")
        self.val_auroc = torchmetrics.AUROC(num_classes=num_classes, task="binary")
        self.val_acc = torchmetrics.Accuracy(num_classes=num_classes, task="binary")
        self.train_ap = torchmetrics.AveragePrecision(
            num_classes=num_classes, task="binary"
        )
        self.val_ap = torchmetrics.AveragePrecision(
            num_classes=num_classes, task="binary"
        )
        self.test_auroc = torchmetrics.AUROC(num_classes=num_classes, task="binary")
        self.test_acc = torchmetrics.Accuracy(num_classes=num_classes, task="binary")
    ...
    def cve(self, d_embedding=None, batch_norm=False):
        ...
        return nn.Sequential(
            nn.Linear(1, d_hidden), nn.Tanh(), nn.Linear(d_hidden, d_embedding)
        )

    def feats_to_input(self, x, batch_size, limits=None):
        times = x["times"]
        # double the event dimension and have frequencies there
        xs_ts, count, times = (
            x["data"].transpose(1, 2),
            x["freq"].transpose(1, 2),
            x["times"],
        )
        xs_ts = xs_ts.nan_to_num(0.0).clip(-3, 3)
        count = count.nan_to_num(0)
        xs_ts = torch.cat((xs_ts, count), dim=-1)
        xs_ts = list(xs_ts)
        ...
        for i, f in enumerate(xs_ts):
            ...
            if self.training and self.aug_noise > 0 and not self.pretrain:
                f[:, :n_vars] += (
                    self.aug_noise * torch.randn_like(f[:, :n_vars]) * f[:, n_vars:]
                )
        ...
        xs_ts = torch.stack(
            [F.pad(t, (0, 0, 0, pad_to - t.shape[0])) for t in xs_ts]
        ).to(self.device)
        xs_times = torch.stack([F.pad(t, (0, pad_to - t.shape[0])) for t in times]).to(
            self.device
        )

        return xs_ts, xs_times, n_timesteps

    def pretrain_prep_batch(self, x, batch_size):
        xs_ts, xs_times, n_timesteps = self.feats_to_input(x, batch_size)
        ...
        for batch_i, n in enumerate(n_timesteps):
            if n < 2:
                ...
                if self.pretrain_masked_steps > n:
                    ...
                    mask_i = self.rng.choice(
                        np.arange(n), size=self.pretrain_masked_steps
                    )
            else:
            ...
            if self.predict_events:
                ...
                y_events_mask.append(
                    xs_ts[batch_i, :, event_mask_i + n_vars].clip(0, 1)
                )
        ...
        if self.pretrain_dropout > 0:
            ...
            keep = torch.cat(
                (keep.tile(1, 2), torch.ones((batch_size, 1), device=keep.device)),
                dim=1,
            )
            xs_ts_clipped *= torch.logical_or(keep.unsqueeze(1), xs_ts_clipped == -1)
        return (
            (xs_ts_clipped, xs_times, n_timesteps),
            y_ts,
            y_ts_masks,
            y_events,
            y_events_mask,
    ...
    def forward(self, x, pretrain=False, representation=False):
        ...
        """
        xs_feats, xs_times, n_timesteps = x
        ...
        if self.predict_events:
            ...
            event_mask_inds = torch.cat(
                (event_mask_inds, event_mask_inds[:, :1, :]), dim=1
            )
        n_obs_inds = (
            xs_feats[:, :, n_vars : n_vars * 2]
            .to(int)
            .clip(0, self.n_obs_embedding.num_embeddings - 1)
        )
        xs_feats[:, :, n_vars : n_vars * 2] = self.n_obs_embedding(n_obs_inds).squeeze(
            -1
        )
        ...
        psi = torch.zeros(
            (xs_feats.shape[0], xs_feats.shape[1] + 1, n_vars, self.d_embedding),
            dtype=xs_feats.dtype,
            device=xs_feats.device,
        )
        ...
        psi[:, -1, :, :] = (
            self.special_embeddings(self.REPRESENTATION_EMBEDDING_KEY.to(self.device))
            .unsqueeze(0)
            .unsqueeze(1)
        )
        mask_inds = torch.cat(
            (
                xs_feats[:, :, -1] == 1,
                torch.zeros(
                    (xs_feats.shape[0], 1), device=xs_feats.device, dtype=torch.bool
                ),
            ),
            dim=1,
        )
        psi[mask_inds, :, :] = self.special_embeddings(
            self.MASKED_EMBEDDING_KEY.to(self.device)
        )
        if self.predict_events:
            psi[event_mask_inds, :] = self.special_embeddings(
                self.MASKED_EMBEDDING_KEY.to(self.device)
            )
        ...
        time_embeddings = torch.cat(
            (
                time_embeddings,
                self.full_rep_embedding.weight.T.unsqueeze(0).expand(
                    xs_feats.shape[0], -1, -1
                ),
            ),
            dim=1,
        )
        for layer_i, (event_transformer, time_transformer) in enumerate(
            zip(self.event_transformers, self.time_transformers)
        ):
            et_out_shape = (psi.shape[0], psi.shape[2], psi.shape[1], psi.shape[3])
            embeddings = psi.transpose(1, 2).flatten(
                2
            ) + self.full_event_embedding.weight.unsqueeze(0)
            event_outs = (
                event_transformer(embeddings).view(et_out_shape).transpose(1, 2)
            )
        ...
        if self.fusion_method == "rep_token":
            ...
            if self.pretrain_masked_steps > 1:
                ...
                z_ts = torch.stack(
                    z_ts
                )  # batch size x pretrain_masked_steps x d_embedding
            else:
                masked_ind = xs_feats[:, :, -1:]
                z_ts = []
                for i in range(transformed.shape[0]):
                    z_ts.append(
                        transformed[i, torch.nonzero(masked_ind[i].squeeze() == 1), :]
                    )
        ...
        if pretrain:
            rep_token_head = torch.tile(
                transformed[:, 0, :].unsqueeze(1),
                (1, self.masked_transform_timesteps, 1),
            )
            y_hat_presence = (
                self.pretrain_presence_proj(z).squeeze()
                if self.pretrain_presence
                else None
            )
            y_hat_value = (
                self.pretrain_value_proj(z).squeeze(1) if self.pretrain_value else None
            )
            z_events = []
            ...
            if self.predict_events:
                for i in range(event_mask_inds.shape[0]):
                    z_events.append(
                        psi[i][event_mask_inds[i].nonzero(as_tuple=True)].flatten()
                    )
                z_events = torch.stack(z_events)
                y_hat_events = self.predict_events_proj(z_events).squeeze()
                y_hat_events_presence = (
                    self.predict_events_presence_proj(z_events).squeeze()
                    if self.pretrain_presence
                    else None
                )
    ...
    def configure_optimizers(self):
        optimizers = [
            torch.optim.AdamW(
                [p for l in self.modules() for p in l.parameters()],
                lr=self.lr,
                weight_decay=self.weight_decay,
                eps=1e-4 if self.half_dtype else 1e-8,
            )
        ]
        return optimizers
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.pretrain:
            x_pretrain, y, mask, y_events, y_events_mask = self.pretrain_prep_batch(
                batch, batch_size
            )
            (
                y_hat_value,
                ...
                y_hat_events,
                y_hat_events_presence,
            ) = self.forward(x_pretrain, pretrain=True)
                ...
                if self.pretrain_masked_steps > 1:
                    for i in range(self.pretrain_masked_steps):
                        loss += self.pretrain_loss(
                            y_hat_value[:, i] * mask[:, i], y[:, i] * mask[:, i]
                        )
            ...
            if self.pretrain_presence:
                if self.pretrain_masked_steps > 1:
                    ...
                    for i in range(self.pretrain_masked_steps):
                        presence_loss += (
                            self.pretrain_presence_loss(
                                y_hat_presence[:, i], mask[:, i]
                            )
                            * self.pretrain_presence_weight
                        )
                    presence_loss /= self.pretrain_masked_steps
                else:
                    presence_loss = (
                        self.pretrain_presence_loss(y_hat_presence, mask)
                        * self.pretrain_presence_weight
                    )
                loss += presence_loss
            if self.predict_events:
                if self.pretrain_value:
                    loss += self.pretrain_loss(
                        y_hat_events * y_events_mask, y_events * y_events_mask
                    )
                if self.pretrain_presence:
                    loss += (
                        self.pretrain_presence_loss(
                            y_hat_events_presence, y_events_mask
                        )
                        * self.pretrain_presence_weight
                    )
        else:
            y_hat = self.forward(self.feats_to_input(batch, batch_size))
            y = batch["outcome"]
            ...
            self.train_acc.update(y_hat, y.to(int))
    ...
    def validation_step(self, batch, batch_idx):
        ...
        if self.pretrain:
            ...
            self.val_acc.update(y_hat, y.to(int).to(self.device))
    ...
    def on_train_epoch_end(self):
        if not self.pretrain:
            self.log(
                "train_auroc", self.train_auroc, sync_dist=True, rank_zero_only=True
            )
            self.log(
                "train_acc",
                self.train_acc,
    ...
    def on_validation_epoch_end(self):
        if not self.pretrain:
            print(
                "val_auroc", self.val_auroc.compute(), "val_ap", self.val_ap.compute(), "val_acc", self.val_acc.compute(),
            )
    ...
    def test_step(self, batch, batch_idx):
        ...
        y = batch["outcome"]
        if self.save_representation:
            y_hat, z = self.forward(self.feats_to_input(batch, batch_size))

            print("saving representations...")
            with open(self.save_representation, "ab") as f:
                if y.ndim == 1:
                    np.savetxt(
                        f, np.concatenate([z.cpu(), y.unsqueeze(1).cpu()], axis=1)
                    )
                else:
        ...
        self.test_acc.update(y_hat, y.to(int).to(self.device))
        self.log(
            "test_acc",
            self.test_acc,
            on_epoch=True,
        ...
        self.log(
            "test_ap", self.test_ap, on_epoch=True, sync_dist=True, rank_zero_only=True
        )

        return loss, self.test_auroc, self.test_ap, self.test_acc
    ...
    def on_load_checkpoint(self, checkpoint):
        ...
        for k in state_dict:
            if k in model_state_dict:
                if (
                    k.startswith("head")
                    and state_dict[k].shape != model_state_dict[k].shape
                ):
                    print(
