...
def initialize_pretrain_duett(cfg: DuettModelConfig):
    ...
    return DuettModule(
        d_time_series_num=d_time_series_num, d_target=d_target,
        half_dtype=cfg.half_dtype, d_static_num=2,
...
class DuettModule(pl.LightningModule):
    ...
    def feats_to_input(self, x, batch_size, limits=None):
        ...
        xs_ts = list(xs_ts)
        xs_static = [torch.zeros((self.d_static_num), device=self.device)] * len(xs_ts)
