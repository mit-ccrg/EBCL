...
def strats_finetune_ray_app(cfg: StratsFinetuneConfig):
    ...
    strats_model = StratsModule.load_from_checkpoint(cfg=cfg.model_config, checkpoint_path=best_checkpoint_dir + "/checkpoint.ckpt")
    ...
    dm.setup()
    results = trainer.test(model=strats_model, datamodule=dm, ckpt_path=best_checkpoint_dir + "/checkpoint.ckpt")[0]
...
if __name__ == "__main__":
    # python -m src.train.strats.strats_finetune_ray dataloader_config.encounter_set=MORTALITY
    logging.basicConfig()
    logging.getLogger().setLevel(logging.WARN)
    strats_finetune_ray_app()
