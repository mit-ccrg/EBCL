...
from src.train.strats.strats_train_pl import get_trainer
...
@hydra_dataclass
class StratsFinetuneConfig(SupervisedTrainConfig):
    defaults: List[Any] = field(default_factory=lambda: defaults)
    batch_size: int = 256
    logger_config: LoggerConfig = LoggerConfig(
        save_dir="/storage/x-x-x/results/strats_finetune", name="ray_strats_finetune"
    )
    ...
    def get_output_path(self):
        ...
        if self.pretrain_ckpt is not None:
            ...
            suffix += "_debug"
        suffix += f"_forecast{self.forecast_window}"
...
def finetune_strats(
    ...
    if cfg.pretrain_ckpt is None:
        finetune_model = StratsModule(cfg=cfg.model_config)
    else:
        finetune_model = StratsModule.initialize_finetune(cfg.model_config, checkpoint_pth)
...
def strats_finetune_app(cfg: StratsFinetuneConfig):
...
if __name__ == "__main__":
    ...
    # python -m src.train.strats.strats_finetune name=test_strats_finetune max_epochs=2 dataloader_config.num_cpus=4 half_dtype=True modalities=PRE_AND_POST dataloader_config.encounter_set=MORTALITY debug=True
