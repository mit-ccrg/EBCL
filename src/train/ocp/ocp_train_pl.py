...
from src.configs.dataloader_configs import CacheMode, OCPDataloaderConfig, SupervisedView
...
from src.data.dataloaders.ocp_dataloader import OCPDataLoaderCreator
...
class OCPLightningDataset(pl.LightningDataModule):
    def __init__(self, ocp_dataloader_creator: CachedDataLoaderCreator):
        super().__init__()
        self.dataloader_creator = ocp_dataloader_creator
...
def train_ocp(
    ...
    ocp_dataloader_creator = OCPDataLoaderCreator(cfg.dataloader_config)

    dm = OCPLightningDataset(ocp_dataloader_creator)
    dm.setup()

    logging.info("Initialize Model...")
    pretrain_model = OCPModule.initialize_pretrain(cfg.model_config)
...
@hydra_dataclass
class OcpPretrainTuneConfig(PretrainConfig):
    defaults: List[Any] = field(default_factory=lambda: defaults)
    pretrain_method: PretrainMethod = PretrainMethod.EBCL
    logger_config: LoggerConfig = LoggerConfig(
        save_dir="/storage/x-x-x/results/ocp_pretrain", name="ray_ocp_pretrain"
    )
    dataloader_config: OCPDataloaderConfig = OCPDataloaderConfig(
    ...
    def __post_init__(self):
        ...
        assert self.modalities == SupervisedView.EARLY_FUSION  # OCP Pretraining requires EARLY Fusion
        if isinstance(self.dataloader_config, OCPDataloaderConfig):
...
def ocp_train_app(cfg: OcpPretrainTuneConfig):
    ...
    assert isinstance(cfg.dataloader_config, OCPDataloaderConfig)
...
if __name__ == "__main__":
    # Test ocp admission pretraining
    # python -m src.train.ocp.ocp_train_pl name=test_ocp_pretrain max_epochs=2 dataloader_config.num_cpus=2 half_dtype=True
    logging.basicConfig()
